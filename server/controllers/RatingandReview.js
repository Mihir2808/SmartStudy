...
exports.getAverageRating = async (req, res) => {
  try {
    ...
    // result is array and first value stores the average rating
    const result = await RatingAndReview.aggregate([
      ...
      },
      {
        $group: {
          _id: null,
          averageRating: { $avg: "$rating" }, // used avg operator to take avg from ratings
        },
      },
    ])
...
exports.getAllRatingReview = async (req, res) => { 
  try {
    ...
    res.status(200).json({
      success: true,
      message: " All Reviews send successfully",
      data: allReviews,
    })
  } catch (error) {
    console.error(error)
